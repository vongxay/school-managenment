...
const fetchUsers = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      error.value =
        response.data.message || "ເກີດຂໍ້ຜິດພາດໃນການດຶງຂໍ້ມູນຜູ້ໃຊ້";
    }
  } catch (err: any) {
    console.error("Error fetching users:", err);
    error.value =
      err.response?.data?.message || "ເກີດຂໍ້ຜິດພາດໃນການດຶງຂໍ້ມູນຜູ້ໃຊ້";
  } finally {
...
const generateLevelId = () => {
  ...
  const maxId = Math.max(...users.map((l) => parseInt(l.id)));
...
const saveUser = async () => {
  ...
  try {
    ...
    error.value =
      err.response?.data?.message || "ເກີດຂໍ້ຜິດພາດໃນການບັນທຶກຂໍ້ມູນຜູ້ໃຊ້";
    alert(error.value);
  } finally {
    isLoading.value = false;
  }
...
const deleteUser = async () => {
  if (
    selectedUser.value &&
    confirm(`ທ່ານແນ່ໃຈບໍວ່າຕ້ອງການລຶບຜູ້ໃຊ້ ${selectedUser.value.name}?`)
  ) {
    isLoading.value = true;
    error.value = null;
    ...
    try {
      ...
      error.value =
        err.response?.data?.message || "ເກີດຂໍ້ຜິດພາດໃນການລຶບຂໍ້ມູນຜູ້ໃຊ້";
      alert(error.value);
    } finally {
      isLoading.value = false;
    }
...
<template>
  <div class="grid grid-cols-3 gap-4 bg-white rounded-lg p-4">
    ...
    <div
      v-if="error"
      class="col-span-3 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4"
    >
      {{ error }}
    </div>
    ...
    <div
      v-if="isLoading"
      class="col-span-3 flex justify-center items-center p-8"
    >
      <div
      ...
      <div class="border-r pr-4 col-span-1">
        ...
        <div class="mb-4 relative">
          ...
          <input
            ...
            class="w-full px-3 py-2 border border-gray-300 rounded pr-10"
            :placeholder="
              selectedUser ? 'ປ່ອຍວ່າງໄວ້ເພື່ອບໍ່ປ່ຽນ' : 'ປ້ອນລະຫັດຜ່ານ'
            "
          />
          <span
          ...
          >
            ...
            >
              <path
                ...
                stroke-linejoin="round"
                stroke-width="2"
                d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.542-7a10.05 10.05 0 012.177-3.309m2.204-1.847A9.953 9.953 0 0112 5c4.477 0 8.267 2.943 9.541 7a9.96 9.96 0 01-1.249 2.527M15 12a3 3 0 11-6 0 3 3 0 016 0z"
              />
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
        ...
        <div class="mb-4">
          ...
          <div class="flex items-center">
            ...
            <label
              for="active-status"
              class="ml-2 py-2 px-3 bg-gray-100 rounded"
            >
              {{ formUser.active ? "Y" : "N" }}
            </label>
          </div>
        ...
        <div class="mb-8">
          ...
          <div class="border border-gray-300 rounded p-2 flex justify-center">
            ...
            >
              ...
              <path
                d="M50,95 C70,95 85,80 85,65 L85,61 C85,53 75,45 50,45 C25,45 15,53 15,61 L15,65 C15,80 30,95 50,95 Z"
                fill="#94a3b8"
              />
            </svg>
          </div>
          <label
            class="mt-2 w-full px-3 py-2 bg-gray-200 text-black rounded hover:bg-gray-300 cursor-pointer text-center block"
          >
            ອັບໂຫລດຮູບພາບ
            <input
              type="file"
        ...
        <div class="grid grid-cols-2 gap-4">
          <button
            ...
            :class="{
              'cursor-not-allowed opacity-60': isLoading || !selectedUser,
            }"
        ...
        <div class="max-h-72 overflow-y-auto">
          <div
            ...
            :class="[
              ...
              selectedUser?.id === user.id ? 'bg-blue-600 text-white' : index%2 !== 0 ? 'bg-gray-100' : '',
            ]"

...
const selectedLevelObj = ref<SelectedYear>({
  id: "",
  name: "",
});
...
const findTuitionByLevelAndYear = (
  level: string,
  year: string
): Tuition | undefined => {
...
const searchTuitionByLevelAndYear = async (
  ...
  try {
    ...
    const response = await axios.get(
      `${API_URL}/tuitions?level=${encodeURIComponent(
        level
  ...
  try {
    ...
    if (selectedTuition.value) {
      ...
      const response = await axios.put(
        `${API_URL}/tuitions/${selectedTuition.value.id}`,
        formTuition
      );
      if (response.data.success) {
        const index = tuitions.findIndex(
          (t) => t.id === selectedTuition.value?.id
        );
        if (index !== -1) {
...
const deleteTuition = async () => {
  ...
  if (
    !confirm(
      `ທ່ານແນ່ໃຈບໍ່ວ່າຕ້ອງການລຶບຂໍ້ມູນຄ່າຮຽນ ${selectedTuition.value.name}?`
    )
  ) {
    return;
  }
...
watch(
  ...
  (newVal) => {
    console.log("Selected Year Object:", newVal);
    if (newVal) {
      formTuition.year = newVal.name; // Access the 'name' property of SelectedYear
    } else {
      formTuition.year = "";
    }
    updateName();
...
watch(
  () => selectedLevelObj.value, // Access the value of the ref
  (newVal) => {
    console.log("Selected Year Object:", newVal);
    if (newVal) {
      formTuition.level = newVal.name; // Access the 'name' property of SelectedYear
    } else {
      formTuition.level = "";
    }
    updateName();
...
const checkForExistingTuition = () => {
  if (formTuition.level && formTuition.year && !selectedTuition.value) {
    const existingTuition = findTuitionByLevelAndYear(
      formTuition.level,
      formTuition.year
    );
    if (existingTuition) {
      ...
      alert(
        `ມີຄ່າຮຽນສຳລັບ ${formTuition.level} ໃນສົກຮຽນ ${formTuition.year} ແລ້ວ`
      );
...
watch(
  [() => formTuition.level, () => formTuition.year],
  checkForExistingTuition
);
...
<template>
  <div class="flex">
    ...
    <div
      v-if="errorMessage"
      class="w-full bg-red-100 text-red-700 p-2 mb-4 rounded"
    >
      {{ errorMessage }}
    </div>
    ...
    <div class="w-1/3 p-4 border-r">
      ...
      <div class="mb-4">
        ...
        <div class="flex items-center space-x-2">
          <input
            type="text"
            :value="selectedLevelObj.id || '000'"
            class="w-24 px-3 py-2 border border-gray-300 rounded cursor-not-allowed"
            readonly
            disabled
          />
          <!-- formTuition.level  -->
          <select
            v-model="selectedLevelObj"
            @change="updateName"
            :label="formTuition.level"
      ...
      <div class="flex space-x-4">
        <button
          ...
          :class="{
            'cursor-not-allowed opacity-60': isLoading || !selectedTuition,
          }"
    ...
    <div class="w-2/3 p-4">
      ...
      <div v-if="showTuitionSummary" class="mb-4 border p-2 bg-gray-50 rounded">
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full">
            ...
            <tbody>
              <tr
                v-for="row in tuitionSummaryData"
                :key="row.level"
                class="border-b"
              >
      ...
      <div class="border">
        ...
        <!-- Empty state -->
        <div
          v-else-if="filteredTuitions.length === 0"
        ...
        <div v-else class="max-h-96 overflow-y-auto">
          <div
            v-for="(tuition, index) in filteredTuitions"
            :key="index"
